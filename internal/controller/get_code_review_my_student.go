...
func init() {
	registerMethod(func(g echoswagger.ApiGroup, c *AdapterController) {
		g.POST("/GetCodeReviewMyStudent", c.Handle_GetCodeReviewMyStudent).
			...
			AddParamBody(requests.GetCodeReviewMyStudent_Variables{}, "variables", "Request variables", true).
			AddResponse(http.StatusOK, "Success", requests.GetCodeReviewMyStudent_Data{}, nil)
	})
}

func (a *AdapterController) Handle_GetCodeReviewMyStudent(c echo.Context) (err error) {
	var data struct {
		Variables requests.GetCodeReviewMyStudent_Variables `json:"variables"`
	}
